...
- Using `INT 2Eh` seemed like a smart move - it gets commands to execute in `COMMAND.COM`! However, `COPY * 4` does not work well - apparently you have to run `COPY *.* 4` which ends up not saving a lot of bytes. Trying to print out `4` or returning an exit code of `4` ends up with more bytes.
