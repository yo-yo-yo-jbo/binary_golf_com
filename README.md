...
- Using `INT 2Eh` seemed like a smart move - it gets commands to execute in `COMMAND.COM`! However, it didn't seem to work. Also, `COPY * 4` does not work well - apparently you have to run `COPY *.* 4` which ends up not saving a lot of bytes.
